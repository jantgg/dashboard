...
function TareasHoy() {
  ...
  return (
    <section>
      <h2 className="tittletareasTH">Tareas para hoy</h2>
      <ul className="listatareasTH">
        {tareasParaHoy.map((tarea) => (
          <li key={tarea._id} className="tareaTH">
            {/* <AiOutlineWarning className="warningTH" /> */}
            <span className="tareanombreTH"> {tarea.titulo}</span>
            <div className="buttonstareaTH">
              {" "}
              <button
              className="checkTH"
              ...
              <button className="trashTH" onClick={() => deleteTarea(tarea._id)}>
                <FaRegTrashAlt  />
              </button>
            </div>
          </li>
